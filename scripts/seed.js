...
async function main() {
  let dappAirBnbContract

  try {
    const contractAddresses = fs.readFileSync('./contracts/contractAddress.json', 'utf8')
    const { dappAirBnbContract: dappAirBnbAddress } = JSON.parse(contractAddresses)

    dappAirBnbContract = await ethers.getContractAt('DappAirBnb', dappAirBnbAddress)
    const dates1 = [1678492800000, 1678579200000, 1678665600000]

    // Process #1
    await Promise.all(
      generateFakeApartment(dataCount).map(async (apartment) => {
        await createApartments(dappAirBnbContract, apartment)
      })
    )
    ...
    await Promise.all(
      Array(dataCount)
        .fill()
        .map(async (_, i) => {
          await bookApartments(dappAirBnbContract, i + 1, dates1)
        })
    )
